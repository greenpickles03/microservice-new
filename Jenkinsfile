pipeline {
  ...
  stages {
    stage('Build service-registry') {
      steps {
        sh 'cd service-registry && mvn install package'
      }
    }

    stage('Build edge-service') {
      steps {
        sh 'cd edge-service && mvn install package'
      }
    }

    stage('Build config-server') {
      steps {
        sh 'cd config-server && mvn install package'
      }
    }

    stage('Build department-service-core'){
        steps {
            sh 'cd department-service-core && mvn install package'
        }
    }

    stage('Build employee-service-core'){
        steps {
            sh 'cd employee-service-core && mvn install package'
        }
    }
